...
protected void decide_set_self_weapon(string, object, object);
protected void get_finish_self_weapon(object, object);
protected void rong_he(object, object);
protected void pei_key(string arg,object me,string *domains);
...
"whip" : "鞭(whip) [需 %d 斤原料]\n",
"staff" : "杖(staff) [需 %d 斤原料]\n",
"fan" : "扇(fan) [需 %d 斤原料]\n",
"brush" : "笔(brush) [需 %d 斤原料]\n",
"hook" : "钩(hook) [需 %d 斤原料]\n",
"fork" : "叉(fork) [需 %d 斤原料]\n",
"bow" : "弓(bow) [需 %d 斤原料]\n",
]),
...
protected int do_nothing;

protected void set_weapon_ability(string *arg)
{
	...
	foreach(string tmp in arg)
		if(!stringp(tmp) || undefinedp(res_cost[tmp]))
			error("set_weapon_ability: 类型错误。\n");

	make_weapon_ability = arg;
}

protected void set_ability_level( int n )
{
	if(n > 0)
		ability_level = n;
}
...
void init()
{
	...
	add_action("do_pei","pei"); /* 配玩家私宅的钥匙.*/
...
int do_pei(string arg)
{
	...
	if( ! (homes = PLAYERHOME_D->user_have_home(me->query("id"))) )
		return notify_fail("你还没有自己的住宅配什么钥匙呀！\n");
	...
	else
	{
		...
		write("你在以下"+chinese_number(n)+"处地区都有自己的住宅，你要配哪一处的钥匙：\n"+out+"\n");
		input_to( (: pei_key :),me, domains );
	}
...
protected void pei_key(string arg,object me,string *domains)
{
	...
	ob = new(sprintf("%s%s/%s_key",PHOME_DIR,domains[n],me->query("id")));
	if(!ob)
	{
		write("你的家门钥匙出现错误，请向巫师报告。\n");
		return;
	}

	if(!f = player_pay(me,KEY_VALUE) || f == 2)
	{
		write(sprintf("配家门钥匙每把需要%s，回去拿够了现钱再来吧。\n",
			chinese_value(KEY_VALUE)));
		destruct(ob);
		return;
	}
	...
	{
		destruct(ob);
		write("你身上已经拿不了更多的东西了，下次再来配吧。");
		return;
	}

	write(name()+"说道：“钥匙配好了，千万保管好。”\n");
	write(sprintf("%s给你一把%s。\n",name(),ob->name()));
}
...
//		return notify_fail("巫师不允许打造兵器。\n");
	...
	if(!arg)	// 初始打造兵器
	{
		...
		out = "可打造的兵器种类：\n";
		for(int i=0; i<sizeof(make_weapon_ability); i++)
			out += sprintf((string)i+1+". "+res_cost_desc[make_weapon_ability[i]],
				res_cost[make_weapon_ability[i]]);
		out += sprintf("请选择你要打造的兵器类型[1-%d, q退出]：\n", sizeof(make_weapon_ability));
		write(out);
...
protected void select_weapon_class(string str, object me, string out)
{
	...
	tell_object(me, "你是否要自己设定兵器的属性？[y/n, q退出]");
	input_to( (: decide_set_self_weapon :), me, ob);
}

protected void decide_set_self_weapon(string str, object me, object ob)
{
	...
	switch(str[0])
	{
		case 'y':
		case 'Y':
			get_self_weapon_name(0, me, ob);
			break;
		case 'n':
		case 'N':
			get_finish_self_weapon(me, ob);
			break;
		default:
			tell_object(me, "\n你是否要自己设定兵器的属性？[y/n, q退出]");
			input_to( (: decide_set_self_weapon :), me, ob);
...
protected void get_finish_self_weapon(object me, object ob)
{
	object *inv;
