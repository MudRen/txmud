// move.c
// Modified by Find.

#pragma optimize

#include <ansi.h>
#include <dbase.h>

static int max_items=20;
nomask void set_max_items(int n) {max_items=n;}
nomask int query_max_items() {return max_items;}

static int weight = 0;
static int encumb = 0, max_encumb = 0;

nomask int query_encumbrance() { return encumb; }
nomask int over_encumbranced() { return encumb > max_encumb; }

nomask int query_max_encumbrance() { return max_encumb; }
nomask void set_max_encumbrance(int e) { max_encumb = e; }
nomask void add_encumbrance(int w)
{
	encumb += w;
	if( encumb < 0 )
		log_file("move.bug", sprintf("%O encumbrance underflow.\n", this_object()));
	if( encumb > max_encumb ) this_object()->over_encumbrance();
	if( environment() ) environment()->add_encumbrance(w);
}

void over_encumbrance()
{
	if( !interactive(this_object()) ) return;
	tell_object(this_object(), "你的负荷过重了！\n");
}

nomask int query_weight() { return weight; }

nomask void set_weight(int w)
{
	if( !environment() ) {
		weight = w;
		return;
	}
	if( w!=weight ) environment()->add_encumbrance( w - weight );
	weight = w;
}


// This is the "current" weight of an object, which is used on weight
// checking in move().
nomask int weight()
{
	if(wizardp(this_object()))
		return 0;
	else
		return weight + encumb;
}

private int do_move_sort(object ob1,object ob2)
{
	if(ob1->is_character() && !ob2->is_character())
		return -1;
	if(!ob1->is_character() && ob2->is_character())
		return 1;
	return 0;
}

private int do_move_filter(object ob)
{
	if(ob == this_object())
		return 0;
	if(ob->query("no_show"))
		return 0;
	if(!this_object()->visible(ob))
		return 0;
	return 1;
}

varargs int move(mixed dest, int silently)
{
	object ob, env;
        object *inv;
        string str;
	mapping exits;
        //int i;


	// If we are equipped, unequip first.
	if( query("equipped") && !this_object()->unequip() )
		return notify_fail("你没有办法取下这样东西。\n");

	// Find the destination ob for moving.
	if( objectp(dest) )
		ob = dest;
	else if( stringp(dest) )
	{
		call_other(dest, "???");
		ob = find_object(dest);
		if(!ob) return notify_fail("move: destination unavailable.\n");
	} else
		return notify_fail(sprintf("move: invalid destination %O.\n", dest));

	// Check if the destination ob can hold this object.
	// Beforce checking it, we check if the destination is environment of
	// this_object() (or environment of its environment). If it is, then
	// this could be like get something from a bag carried by the player.
	// Since the player can carry the bag, we assume he can carry the this
	// object in the bag and encumbrance checking is unessessary.
	env = this_object();

        if(env==ob) return notify_fail("移动错误！\n");

	while(env = environment(env)) if( env==ob ) break;
	if( !env && (int)ob->query_encumbrance() + weight()
		> (int)ob->query_max_encumbrance() ) {
		if( ob==this_player() )
			return notify_fail( this_object()->name() + "对你而言太重了。\n");
		else
			return notify_fail( this_object()->name() + "对" + ob->name() + "而言太重了。\n");
	}

	if(!(this_object()->is_character())
	&& !(this_object()->is_pawn_stamp())	// for pawn_stamp. by Find.
	&& (sizeof(filter_array(all_inventory(ob),(: !$1->is_pawn_stamp() :)) )
		>=ob->query_max_items()))
	{
		if(ob==this_player())
			return notify_fail("你身上装不下更多东西了。\n");
		else if(userp(ob))
			return notify_fail(ob->name()+"身上装不下更多东西了。\n");
		else
		{
		string name=ob->name();
		if(!name) name=ob->query("short");
		if(!name) name=ob->short();
		return notify_fail(name+"已经太拥挤了。\n");
		}
	}

#ifdef WIZARD_FLAG
	if(sscanf(file_name(this_object()), "/wiz/%*s"))
		this_object()->set_flag(1);

	if( (userp(ob) && wizardp(ob)) || sscanf(base_name(ob),"/wiz/%*s") )
		if(!userp(this_object()))
			this_object()->set_flag(1);
#endif

	// Move the object and update encumbrance
	// Modified by Find. for light.
	if( environment() )
	{
		if(this_object()->is_character() && environment()->is_lightroom())
			environment()->char_leave_room(this_object());
		environment()->add_encumbrance( - weight());
	}
	move_object(ob);
	ob->add_encumbrance(weight());
	if(this_object()->is_character() && ob->is_lightroom())
		ob->char_comeon_room(this_object());

	// If we are players, try look where we are.
	if( interactive(this_object())	// are we linkdead?
	&& living(this_object())	// are we still concious?
	&& !silently )
	{
		if(environment()->query_dark_level())
			tell_object(this_object(),WHT"周围黑漆漆的一片，什么也看不见。\n"NOR);

                else if( query("env/brief") == 1 )
		{
			str = HIY "【"+environment()->query("short") + HIY"】" NOR;
			if(mapp(exits = environment()->query("exits")) && sizeof(exits))
			{
				string *dirs = keys(exits);
				str += " [";
				for(int n=0;n<sizeof(dirs);n++)
					str += " "+capitalize(dirs[n]);
				str += " ]";
			}
			str += "\n";
                        tell_object(this_object(), str);
		}

                else if( query("env/brief") == 2)
                {
			mixed groups;
			int n;

			str = HIY "【"+environment()->query("short") +HIY "】" NOR;
			if(mapp(exits = environment()->query("exits")) && sizeof(exits))
			{
				string *dirs = keys(exits);
				str += " [";
				for(n=0;n<sizeof(dirs);n++)
					str += " "+capitalize(dirs[n]);
				str += " ]";
			}
			str += "\n";
			inv = filter_array(all_inventory(environment()),(: do_move_filter :));
			inv = sort_array(inv,"do_move_sort");
			groups = unique_array(inv,(: sprintf("%s%s",base_name($1),$1->name()) :));

			if(sizeof(groups))
			{
				foreach(object *group in groups)
				{
					if( (n=sizeof(group)) == 1)
						str += sprintf("  %s\n",group[0]->short(-1));
					else if(userp(group[0]))
					{
						foreach(object user in group)
							str += sprintf("  %s\n",user->short(-1));
					}
					else
						str += sprintf("  %s%s%s\n",chinese_number(n),
						group[0]->query("unit"),
						group[0]->query("base_cname")?
						sprintf("%s(%s)",group[0]->query("base_cname"),
						capitalize(group[0]->query("id"))):
						group[0]->short());
				}
			}
	               	tell_object(this_object(), str);
                }

		else
			command("look");
	}

	return 1;
}

void remove(string euid)
{
	object default_ob;

	if( !previous_object()
	||	base_name(previous_object()) != SIMUL_EFUN_OB )
		error("move: remove() can only be called by destruct() simul efun.\n");

	if( userp(this_object()) && euid!=ROOT_UID ) {
		log_file("destruct", sprintf("%s attempt to destruct user object %s (%s)\n",
			euid, this_object()->query("id"), ctime(time())));
		error("你(" + euid + ")不能摧毁其他的使用者。\n");
	} else if( this_object()->query("equipped")) {
		if(	!this_object()->unequip() )
			log_file("destruct", sprintf("Failed to unequip %s when destructed.\n",file_name(this_object())));
	}

	// We only care about our own weight here, since remove() is called once
	// on each destruct(), so our inventory (encumbrance) will be counted as
	// well.
	if( environment() )	environment()->add_encumbrance( - weight );
	if( default_ob = this_object()->query_default_object() )
		default_ob->add("no_clean_up", -1);
}

int move_or_destruct( object dest )
{
	if( userp(this_object()) ) {
		tell_object(this_object(), "一阵时空的扭曲将你传送到了一个陌生的地方....\n");
		move(VOID_OB);
	}
}
