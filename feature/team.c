	...
	|| (query("pursuer") && this_object()->is_killing(ob->query("id")))) 
	{
		...
		&& ob->query_temp("on_rided") == this_object())
		{
			if(!environment(this_object())->valid_leave(this_object(),dir))
			{
				...
				tell_object(ob,"你的坐骑没能和你一起过来。\n");
				return 1;
			}
			else
			{
				tell_object(ob,"你骑着"+this_object()->name()+"走了过来。\n");
		...
		else if(ob->query("ride"))
		{
			if(this_object()->query_temp("on_rided"))
			{
				...
				{
				tell_object(this_object(),ob->name()+"的脚力太快，你的坐骑追不上了。\n");
				return 1;
				}
			}
			...
			{
				tell_object(this_object(),ob->name()+"的脚力太快，你追不上了。\n");
				return 1;
			}
		}
		...
		else if(ob->query_temp("on_rided"))
		{
			if( this_object()->query_temp("on_rided"))
			{
				...
				{
				tell_object(this_object(),ob->name()+"的坐骑脚力太快，你的坐骑追不上了。\n");
				return 1;
				}
			}
			else if((int)this_object()->query_skill("dodge",1)
			...
			{
				tell_object(this_object(),ob->name()+"的坐骑脚力太快，你追不上了。\n");
				return 1;
			}
		}
		return follow_path(dir);
