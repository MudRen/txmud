...
int look_item(object me, object obj)
{
        ...
        // 普通9个★，极品多3个金黄色的★
        // ★★★★★★★★★☆☆☆
        if(stars=obj->query("attribute/stars"))        // by aiai
        {
                look_msg+=HIC "  ";
                if (stars<9)
                {
        ...
        if(mapp(req = obj->query("required")))  // by aiai
        {
                string i_type,i_use;
                ...
                // 目前装备限制有：
                // exp,int,str / literate,unarmed,dodge
                if(!undefinedp(req["exp"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  %s这%s%s需要：\n", i_use, obj->query("unit"), i_type);

                        if(me->query("combat_exp")<req["exp"])
                                look_msg += sprintf(RED "      至少有 %d 点经验\n" NOR, req["exp"]);
                        else
                                look_msg += sprintf(CYN "      至少有 %d 点经验\n" NOR, req["exp"]);
                }

                if(!undefinedp(req["int"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  %s这%s%s需要：\n", i_use, obj->query("unit"), i_type);

                        if(me->query("int")<req["int"])
                                look_msg += sprintf(RED "      至少有 %d 点悟性\n" NOR, req["int"]);
                        else
                                look_msg += sprintf(CYN "      至少有 %d 点悟性\n" NOR, req["int"]);
                }

                if(!undefinedp(req["str"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  %s这%s%s需要：\n", i_use, obj->query("unit"), i_type);

                        if(me->query("str")<req["str"])
                                look_msg += sprintf(RED "      至少有 %d 点膂力\n" NOR, req["str"]);
                        else
                                look_msg += sprintf(CYN "      至少有 %d 点膂力\n" NOR, req["str"]);
                }

                // 有新的数值限制，请写在下面


                if(!undefinedp(req["literate"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  %s这%s%s需要：\n", i_use, obj->query("unit"), i_type);

                        if(me->query_skill("literate")<req["literate"])
                                look_msg += sprintf(RED "      至少有 %d 级%s\n" NOR, req["literate"], to_chinese("literate"));
                        else
                                look_msg += sprintf(CYN "      至少有 %d 级%s\n" NOR, req["literate"], to_chinese("literate"));
                }

                if(!undefinedp(req["unarmed"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  %s这%s%s需要：\n", i_use, obj->query("unit"), i_type);

                        if(me->query_skill("unarmed")<req["unarmed"])
                                look_msg += sprintf(RED "      至少有 %d 级%s\n" NOR, req["unarmed"], to_chinese("unarmed"));
                        else
                                look_msg += sprintf(CYN "      至少有 %d 级%s\n" NOR, req["unarmed"], to_chinese("unarmed"));
                }

                if(!undefinedp(req["dodge"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  %s这%s%s需要：\n", i_use, obj->query("unit"), i_type);

                        if(me->query_skill("dodge")<req["dodge"])
                                look_msg += sprintf(RED "      至少有 %d 级%s\n" NOR, req["dodge"], to_chinese("dodge"));
                        else
                                look_msg += sprintf(CYN "      至少有 %d 级%s\n" NOR, req["dodge"], to_chinese("dodge"));
                }
        ...
        if(mapp(attr = obj->query("attribute")))        // by aiai
        {
                string a_type;
                ...
                // 目前的属性有：
                // +damage, +armor, +eff_gin, +eff_kee, +eff_sen, +eff_force
                // +str, +cor, +int, +con, +kar, +per
                // damage_plus, armor_plus
                // 只允许临时增加(非永久性)
                // gin,kee,sen,str,cor...per是特殊属性，出现在普通需鉴定武器、特殊武器、合成武器中
                // lv_kee,lv_force是特殊成长属性（有可能加入lv_str,lv_con...等等）
                if(!undefinedp(attr["damage"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(WHT "      %d 点攻击力\n" NOR, attr["damage"]);
                }

                if(!undefinedp(attr["armor"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(WHT "      %d 点防御力\n" NOR, attr["armor"]);
                }

                // 有新的基本属性，请写在下面，注意：使用WHT颜色


                if(!undefinedp(attr["gin"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点精\n" NOR, attr["gin"]);
                }

                if(!undefinedp(attr["kee"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点气\n" NOR, attr["kee"]);
                }

                if(!undefinedp(attr["sen"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点神\n" NOR, attr["sen"]);
                }

                if(!undefinedp(attr["force"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点内力\n" NOR, attr["force"]);
                }

                if(!undefinedp(attr["str"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点膂力\n" NOR, attr["str"]);
                }

                if(!undefinedp(attr["cor"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点胆识\n" NOR, attr["cor"]);
                }

                if(!undefinedp(attr["int"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点悟性\n" NOR, attr["int"]);
                }

                if(!undefinedp(attr["con"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点根骨\n" NOR, attr["con"]);
                }

                if(!undefinedp(attr["kar"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点福缘\n" NOR, attr["kar"]);
                }

                if(!undefinedp(attr["per"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(HIC "      提高 %d 点容貌\n" NOR, attr["per"]);
                }

                // 有新的特殊属性，请写在下面，注意：特殊成长属性用HIC(亮兰色)显示



                if(!undefinedp(attr["damage_plus"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(CYN "      追加 %d 点攻击力\n" NOR, attr["damage_plus"]);
                }

                if(!undefinedp(attr["armor_plus"]))
                {
                        if(!look_msg)
                                look_msg = sprintf("  这%s%s的属性：\n", obj->query("unit"), a_type);
                        look_msg += sprintf(CYN "      追加 %d 点防御力\n" NOR, attr["armor_plus"]);
                }

                // 有新的追加属性，请写在下面，注意：使用CYN颜色


                if(!undefinedp(attr["lv_damage"]))
                {
                        look_msg += sprintf("  每提高一段，增加 %d 点攻击力\n", attr["lv_damage"]);
                }

                if(!undefinedp(attr["lv_armor"]))
                {
                        look_msg += sprintf("  每提高一段，增加 %d 点防御力\n", attr["lv_armor"]);
                }

                // 有新的成长属性，请写在下面


                if(!undefinedp(attr["lv_damage_plus"]))
                {
                        look_msg += sprintf("  每提高一段，增加 %d 点追加攻击力\n", attr["lv_damage_plus"]);
                }

                if(!undefinedp(attr["lv_armor_plus"]))
                {
                        look_msg += sprintf("  每提高一段，增加 %d 点追加防御力\n", attr["lv_armor_plus"]);
                }

                // 有新的追加成长属性，请写在下面


                if(!undefinedp(attr["lv_kee"]))
                {
                        look_msg += sprintf(HIG "  每提高一段，增加 %d 点气\n" NOR, attr["lv_kee"]);
                }

                if(!undefinedp(attr["lv_force"]))
                {
                        look_msg += sprintf(HIG "  每提高一段，增加 %d 点内力\n" NOR, attr["lv_force"]);
                }

                // 有新的特殊成长属性，请写在下面，注意：特殊成长属性用HIG(亮绿色)显示


                if(!undefinedp(attr["level"]))
                {
                        if (attr["level"]<1)
                                look_msg += sprintf("  这%s%s不能锻造\n", obj->query("unit"), a_type);
                        else
                                look_msg += sprintf("  目前为第 %s 段\n", chinese_number(attr["level"]));
                }
