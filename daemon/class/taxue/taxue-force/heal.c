...
int exert(object me, object target)
{
	...
	if(target==me)
		message_vision(HIW"$N盘膝跌坐，手心相对，全身真气行走周天，头顶笼罩着一丝白雾。\n"NOR,me);
	else
		message_vision(HIG"$N盘膝跌坐，双手抵住$n的后心，全身真气行走周天，头顶笼罩着一丝白雾。\n"NOR,
			me,target);
...
void finish_healing(object me,object target)
{
	...
	target->add("kee",add);
	tell_object(target,HIG"你只觉得一丝清凉之气在体内行走一周，身上的伤痛好多了。\n"NOR);
	message_vision(HIW"$N行功圆满，站了起来。\n"NOR,me);
	me->stop_busy();
}
