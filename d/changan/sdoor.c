...
void create()
{
        set("short", "长安城南门");
        set("long", @LONG
这里是长安城的南门，城上的御林军在来回的巡逻，平时由
这里出入的人并不多。两个城门官悠闲的走来走去。在城门的一
旁帖着一张布告(bugao)。从这里向北是长安城的南安大道。
LONG
        );
	...
	set( "gate_dir",(["south" : __DIR__"out_changan_s"]) );

	// 这是门关上以后看到的房间描述.
	// 规则同房间的长描述。.
	set( "close_look",@LONG
这里是长安城的南门。现在是宵禁时间，城门已经关上了。
