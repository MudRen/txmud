...
{	
	...
	::init();
	if( living(who) && !is_fighting() && !who->query_temp("order") )
	{
...
void greeting(object who)
{
	...
	if(!userp(who) && who->query("is_beggar"))
	{
		message_vision(CYN"$N冲着$n喊道：穷光蛋还想玩姑娘，给老娘滚出去！\n"NOR,this_object(),who);
		who->go_back_direction();
		return;
	}
	...
	{
		if(is_day())
			message_vision( CYN"老鸨冲着$N微笑道：我们这的四朵金花远近闻名，"+
			"请爷天黑了来吧，一定让您好好爽爽。\n"NOR,who);
		else
			message_vision( CYN"老鸨冲着$N微笑道：这位"+RANK_D->query_respect(who)+
			"要不要摆一桌花酒，我把孩子们叫出来陪您？\n"NOR,who);
	}
	if(who->query("gender") == "女性")
		message_vision( CYN"老鸨奇怪的望着$N说道：这位"+RANK_D->query_respect(who)+
			"跑我们这种地方来做什么？\n"NOR,who);
}
