...
LONG
	...
	setup();
	carry_object(__DIR__"obj/t_packet");
	carry_object(__DIR__"obj/shuiran_skirt")->wear();
}
...
void greeting(object ob)
{
	...
	message_vision("\n$N朝$n盈盈一福：奴家来伺候您了。奴叫香玉(xiang yu)。\n",this_object(),ob);
	message_vision("奴的「暖香阁」里，让爷从里到外暖个透。(accept xiang yu)\n",this_object());
}

int accept_object(object who,object ob)
{
	object obj;
	if(!who->query_temp("jiyuan/shangqian") || who->query_temp("jiyuan/shangqian") != "香玉") return 0;
	...
	message_vision(CYN"$N高兴地说道：多谢$n大爷的赏。爷跟我来吧。"NOR,this_object(),who);
...
int do_sleep(string arg)
{
	...
	int i;
	if(!me->query_temp("jiyuan/sleep") || me->query_temp("jiyuan/sleep") != "香玉")
		return 0;
