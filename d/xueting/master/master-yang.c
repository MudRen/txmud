...
void create()
{
	set_name("杨得草", ({ "master yang", "master"}) );
	...
	set("long",@LONG
丐帮雪亭镇分舵的三袋弟子，想入丐帮找他就对了。
LONG );

	create_family("丐帮", 9, "三袋弟子");
	...
	set_skill("stick", 100);
	set_skill("force", 100);
	set_skill("taizu-changquan",80);
	set_skill("taizu-force",80);
	set_skill("gaibang-steps",100);
	set_skill("taizu-gun", 100);
...
//		return;
//	}
	if( (ob->query_skill("begging",1) < 40)
		|| (ob->query_skill("taizu-force",1) < 40) )
		{
