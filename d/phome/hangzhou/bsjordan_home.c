...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","bsjordan");	// 房主的 ID。
	set("master_name","怀德居士"); // 房主的中文名。
	set("exits",([
	"north" : __DIR__"bsjordan_home2",
	"south" : "/d/hangzhou/changane2",
	]));

	call_out("check_room",1);
	set("outdoors","playerhome");
	setup();
        create_door("south", "红漆大门", "north", DOOR_CLOSED,LOCKED,__DIR__"bsjordan_key");
}

string query_save_file()
{
	return HOME_DIR+"hangzhou/bsjordan_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("怀德居士把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
