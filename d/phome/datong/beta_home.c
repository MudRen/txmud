...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","beta");	// 房主的 ID。
	set("master_name","侠骨柔情"); // 房主的中文名。
	set("exits",([
	"west" : __DIR__"beta_home2",
	"east" : "/d/datong/out_datong_s",
	]));
	...
	setup();
        create_door("east", "柳木门", "west", DOOR_CLOSED,LOCKED,__DIR__"beta_key");

}

string query_save_file()
{
	return HOME_DIR+"datong/beta_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("侠骨柔情把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
