...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","tigertu");	// 房主的 ID。
	set("master_name","温柔的老虎"); // 房主的中文名。
	set("exits",([
	"south" : __DIR__"tigertu_home2",
	"north" : "/d/kaifeng/out_kaifeng_w",
	]));

	call_out("check_room",1);
	set("outdoors","playerhome");
	setup();
        create_door("north", "红漆大门", "south", DOOR_CLOSED,LOCKED,__DIR__"tigertu_key");
}

string query_save_file()
{
	return HOME_DIR+"kaifeng/tigertu_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("温柔的老虎把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
