...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","speed");	// 房主的 ID。
	set("master_name","东方破剑"); // 房主的中文名。
	set("exits",([
	"east" : __DIR__"speed_home2",
	"west" : "/d/kaifeng/kai15",
	]));

	call_out("check_room",1);
	set("outdoors","playerhome");
	setup();
        create_door("west", "红漆大门", "east", DOOR_CLOSED,LOCKED,__DIR__"speed_key");
}

string query_save_file()
{
	return HOME_DIR+"kaifeng/speed_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("东方破剑把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
