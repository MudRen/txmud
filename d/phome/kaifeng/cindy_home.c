...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","cindy");	// 房主的 ID。
	set("master_name","辛迪克劳馥"); // 房主的中文名。
	set("exits",([
	"west" : __DIR__"cindy_home2",
	"east" : "/d/kaifeng/kai34",
	]));

	call_out("check_room",1);
	set("outdoors","playerhome");
	setup();
        create_door("east", "红漆大门", "west", DOOR_CLOSED,LOCKED,__DIR__"cindy_key");
}

string query_save_file()
{
	return HOME_DIR+"kaifeng/cindy_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("辛迪克劳馥把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
