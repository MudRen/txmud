...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","jef");	// 房主的 ID。
	set("master_name","大肚和尚"); // 房主的中文名。
	...
	setup();
        create_door("west", "木门", "east", DOOR_CLOSED,LOCKED,__DIR__"jef_key");

}

string query_save_file()
{
	return HOME_DIR+"path2/jef_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("大肚和尚把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
