...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","loveryou");	// 房主的 ID。
	set("master_name","梦幻天使"); // 房主的中文名。
	...
	setup();
        create_door("west", "木门", "east", DOOR_CLOSED,LOCKED,__DIR__"loveryou_key");

}

string query_save_file()
{
	return HOME_DIR+"path2/loveryou_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("梦幻天使把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
