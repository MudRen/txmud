...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","kdk");	// 房主的 ID。
	set("master_name","孙申"); // 房主的中文名。
	set("valid_startroom", 1);
	set("exits",([
	"west"  : "/d/path2/da_hou1",
	]));

	call_out("check_room",1);
	setup();
        create_door("west", "木门", "east", DOOR_CLOSED,LOCKED,__DIR__"kdk_key");

}

string query_save_file()
{
	return HOME_DIR+"path2/kdk_home_data";
}
