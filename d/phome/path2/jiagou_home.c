...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","jiagou");	// 房主的 ID。
	set("master_name","比猪还笨"); // 房主的中文名。
	...
	setup();
        create_door("south", "木门", "north", DOOR_CLOSED,LOCKED,__DIR__"jiagou_key");

}

string query_save_file()
{
	return HOME_DIR+"path2/jiagou_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("比猪还笨把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
