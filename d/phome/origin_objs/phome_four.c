...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","$ID");	// 房主的 ID。
	set("master_name","$N"); // 房主的中文名。
	set("exits",([
	"$BDIR" : __DIR__"$ID_home2",
	"$DIR" : "$EXITOBJ",
	]));
	...
	setup();
        create_door("$DIR", "红漆大门", "$BDIR", DOOR_CLOSED,LOCKED,__DIR__"$KEYOBJ");
}

string query_save_file()
{
	return HOME_DIR+"$SAVEFILE";
}
