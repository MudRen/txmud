...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","jgz");	// 房主的 ID。
	set("master_name","威震天下"); // 房主的中文名。
	set("valid_startroom", 1);
	set("exits",([
	"north"  : "/d/fengtian/out_fengtian_w",
	]));

	call_out("check_room",1);
	setup();
        create_door("north", "木门", "south", DOOR_CLOSED,LOCKED,__DIR__"jgz_key");

}

string query_save_file()
{
	return HOME_DIR+"fengtian/jgz_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("威震天下把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
