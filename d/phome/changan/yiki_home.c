...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","yiki");	// 房主的 ID。
	set("master_name","一晖"); // 房主的中文名。
	set("exits",([
	"east" : __DIR__"yiki_home2",
	"west" : "/d/changan/nroad9",
	]));

	call_out("check_room",1);
	set("outdoors","playerhome");
	setup();
        create_door("west", "红漆大门", "east", DOOR_CLOSED,LOCKED,__DIR__"yiki_key");
}

string query_save_file()
{
	return HOME_DIR+"changan/yiki_home_data";
}
