...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","feig");	// 房主的 ID。
	set("master_name","紫雨冷月"); // 房主的中文名。
	set("valid_startroom", 1);
	set("exits",([
	"north"  : "/d/path1/cc_tch20",
	]));

	call_out("check_room",1);
	setup();
        create_door("north", "木门", "south", DOOR_CLOSED,LOCKED,__DIR__"feig_key");

}

string query_save_file()
{
	return HOME_DIR+"path1/feig_home_data";
}
...
int do_banzou(string arg)
{
	...
	if(!obs[f])
		map_delete(obs,f);
	message_vision("紫雨冷月把$n从屋子里扔了出去。\n",me,ob);
	destruct(ob);
	set("objects",obs);
