...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","conne");	// 房主的 ID。
	set("master_name","张让"); // 房主的中文名。
	set("valid_startroom", 1);
	set("exits",([
	"south"  : "/d/path1/tch_ft21",
	]));

	call_out("check_room",1);
	setup();
        create_door("south", "木门", "north", DOOR_CLOSED,LOCKED,__DIR__"conne_key");

}

string query_save_file()
{
	return HOME_DIR+"path1/conne_home_data";
}
