...
LONG
	...
	set(SAFE_ENV,1);
	set("master_id","wind");	// 房主的 ID。
	set("master_name","风"); // 房主的中文名。
	set("valid_startroom", 1);
	set("exits",([
	"north"  : "/d/path1/tch_ft6",
	]));

	call_out("check_room",1);
	setup();
        create_door("north", "木门", "south", DOOR_CLOSED,LOCKED,__DIR__"wind_key");

}

string query_save_file()
{
	return HOME_DIR+"path1/wind_home_data";
}
