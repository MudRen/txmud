...
int is_poisoned()
{
	...
	me->apply_condition("yangdu_poison",(int) me->query("kar")+ random(30));
		message("channel:chat",
			HIY"【旁白】"HIC"踏雪山庄突然传来一阵喧闹声：“抓贼呀！有人偷入密室！\n"NOR,users());
...
{
	message("channel:chat",
			HIY"【踏雪山庄】"HIC"踏雪山庄大弟子正在和花枯发禀告经过。踏雪庄主马上到密室去查看。\n"NOR,users());
	call_out("zi_bai",10);
	return 1;
}
...
{
	message("channel:chat",
			HIY"【踏雪山庄】"HIC"踏雪庄主心有余悸的说：“幸好只偷去了那本假的。不过，我在那本假书上涂了我们的"HIR"恙毒。\n"HIC"            那位梁上君子现在恐怕毒发了，正忙着找解药呢。\n"NOR,users());
	call_out("zi_bai2",10);
	return 1;
}
