        ...
        if( objectp(npc = setup_npc(player)) )
        {
                ...
                if ( npc->query("high_level") )
                        for (i=0; i<random(5)+1; i++)
                                new(__DIR__"npc/loulou")->move(room_out);
        }
...
protected object setup_npc(object player)
{
        ...
        zhan->set_skill("ganchan",150+random(100));
                zhan->set_skill("lianhua-blade",150+random(100));
                zhan->set_skill("feilong-quan",150+random(100));
        zhan->map_skill("blade", "lianhua-blade");
                zhan->map_skill("parry", "lianhua-blade");
                zhan->map_skill("dodge", "ganchan");
                zhan->map_skill("unarmed", "feilong-quan");

                exp=1000+random(1000);
                zhan->set("max_kee", exp);
                zhan->set("eff_kee", exp);
                zhan->set("kee", exp);
                ...
                zhan->set("gin", exp);
		zhan->set("force_factor", 50+random(50));
        }
