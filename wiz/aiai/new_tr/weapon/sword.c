...
string query_selling_msg() { return "宝物兵器"; }
...
int set_weapon_attr(int n)
{
        ...
        {
                log_file("set_tr",sprintf("(%s) CALL %s(%s) SET_WEAPON_ATTR %n\n",
...
int is_tr_object() { return 1; }
int is_tr_weapon() { return 1; }
...
nomask mixed set(string prop, mixed data)
{
        ...
        if(pob && (file_name(pob) == "/cmds/wiz/call"))
        {
                log_file("set_tr",sprintf("(%s) CALL %s(%s) SET %s : %O\n",
