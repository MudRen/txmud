...
private void finish_lookup( string host, string number );
private void resolve_callback( string address, string resolved, int key );
private string get_path( int fd, string str );
private int strncmp(string str, string sub, int i);
// Modified by Find.
private string *exclude_addr = ({ "127.0.0.1","210.78.144.10"});

private int muti_login(string name)
{
	if(!stringp(name) || name == "")
		return 0;

	foreach(int file_desc,mapping info in socket_info)
...
private void lose_connection( int fd ) {
	socket_close( fd );
	map_delete( socket_info, fd );
}

private void lose_user( int fd ) {
	socket_close( fd );
	map_delete( socket_info, fd );
	number_of_users--;
}
...
// Modified by Find.
private varargs int check_valid_read( string fname, int fd, int flag ) {
	int res;

	seteuid( socket_info[ fd ][ USER_NAME ] );
	res = (int) MASTER_OB -> valid_read(fname, this_object(),
		(flag)?"stat":"read_file"); //  by Find for valid_read.
	seteuid( getuid() );
...
private int check_valid_write( string fname, int fd ) {
	int res;
...
private mapping *query_connections() {
	mixed *vals;
	mapping *list;
	int count, i;
...
string ls( string path, int column, int fd )
{
	...
	for (i = 0; i < s; i++)
	{
		...
		if (j == -2)
		{
			...
			files[i] = sprintf("-rw%crw-r-- %3d %-8s %-8s %8d %12s %s",
...
private void data_read_callback( int fd, mixed mess ) {
	if ( socket_info[ fd ][ TYPE ] != DOWNLOAD )
		return;
		...
		do
		{
			if(catch( res = write_buffer( UPATH, pos,
			 UDATA[pos..(pos+(BLOCK_SIZE-1))] ) ))
				socket_write( socket_info[ fd ][ PARENT_FD ],"452 Error writing file.\n");
			if (res)
				pos += BLOCK_SIZE;
		} while (res);
...
message("channel:sys",sprintf(HIR"【系统】文件传输服务：%s 从 %s 退出服务。\n"NOR,
UNAME,USITE),filter_array(users(),(: wizardp($1) :)));
...
private void parse_comm( int fd, string str )
{
	...
	switch( lower_case( command[ 0 ] ) )
	{
		case "port": // specify data connection port
			...
			if ( sizeof( command ) < 6 )
				socket_write( fd, "550 Failed command 'port'.\n" );
			else
			{
				string remote_addr = implode( command[ 0..3 ], "." );
				if( (member_array(remote_addr,exclude_addr) != -1)
				&& (member_array(USITE,exclude_addr) == -1) )
				{	// by Find.
					socket_write( fd, "Error: Illegal address.\n" );
					logout( fd );
					lose_user( fd );
					return;
				}
		...
		case "user": // specify user name
			...
			// No Muti_login. by Find.
#ifndef ANONYMOUS_FTP
			if( muti_login(command[ 1 ]) )
			{
...
#endif
			if ( !check_access( command[ 1 ] ,USITE ) ) {
				socket_write( fd, sprintf("530 User %s access denied.\n",
					  command[ 1 ]) );
#ifdef LOG_TIME
				log_file( LOG_FILE, FTP_TIME );
	...
	"331 Guest login ok, send your complete E-mail address as password.\n" );
		...
		case "pass": // specify password
			...
			if ( !check_password( socket_info[fd][ USER_NAME ],
				  implode( command[ 1..sizeof( command ) - 1 ], " " ) ,USITE) ) {
#ifdef ANONYMOUS_FTP
				if (UNAME == "anonymous")
					socket_write( fd,
...
message("channel:sys",sprintf(HIR"【系统】文件传输服务：%s 从 %s 连线进入。\n"NOR,
UNAME,USITE),filter_array(users(),(: wizardp($1) :)));
		...
		case "retr": // retrieve a file
			...
			else if ( !check_valid_read( tmp, fd ) ) // 不可读
			{
		...
		case "list": // give list files in a directory
			...
			/* Modified by Find. For now some ftp client use "LIST -L" command.*/
			if ( sizeof( command ) > 1 )
			{
				string arg;
				if(sscanf(command[1],"%s-L",arg) == 1)
					tmp = get_path(fd,arg);
				else tmp = get_path( fd, command[ 1 ] );
			}
			else
				tmp = socket_info[ fd ][ CWD ];
			...
			// by Find.
			if ( check_valid_read( tmp, fd, 1 ) ) {
				/* Modified by Find. Also return the dirs.*/
				tmp2 = ls( tmp, 0, fd );
				if (tmp2 == "")
					socket_write( fd, "550 No files found.\n");
				else
