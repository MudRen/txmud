...
void setup_human(object ob)
{
	...
	if( userp(ob) || undefinedp(my["max_sen"]) )
	{
		if( my["age"] <= 30 ) my["max_sen"] = 100;
                else if( my["age"] <= 60 ) my["max_sen"] = 100 + (my["age"] - 30) * 5;
                else my["max_sen"] = 250;
	...
	if( !ob->query_weight() ) ob->set_weight(BASE_WEIGHT + (ob->query("str") - 10)* 1500);
}
