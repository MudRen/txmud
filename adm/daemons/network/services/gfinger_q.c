...
#undef DEBUG   // added by Find.

inherit F_CLEAN_UP;

inherit F_DBASE; // added by find.
...
#ifdef DEBUG	// by Find.
	set("channel_id", "网际玩家查找精灵");
	CHANNEL_D->do_channel(this_object(), "sys",
		sprintf("accept a gfinger request from (%s) to finger (%s) in mud (%s).",
		them->query("id"),wiz,mud));
#endif
...
// added by Find.
	...
	{
		tell_object(them,"网际精灵没有载入，无法与其他游戏取得联系。\n");
		return;
	}
// end.
	...
	if (!minfo)
	{
		tell_object(them,HIG "This Mud is not online!\n" NOR);
		return ;
	}
	...
	set("channel_id", "网际玩家查找精灵");
	CHANNEL_D->do_channel(this_object(), "sys",
		sprintf("Sent out a gfinger request to mud (%s).",mud));
#endif
