...
protected void add_door(string str, object who, object env, string *ye, string *yd, string cname)
{
	...
	if(!BANGHUI_D->parse_function_body(ref dcontent, "void", "create", ref df_sect, ref dm_sect, ref de_sect, 0))
	{
	...
	if(!BANGHUI_D->check_include(ref f_sect, "<room.h>"))
		result += "#include <room.h>\n";

	result += sprintf("%s\n%s\n%s\n%s\n",f_sect, m_sect, dstr, e_sect);

	dresult = sprintf("// Room: %s\n\n", dfname);
	if(!BANGHUI_D->check_include(ref df_sect, "<room.h>"))
...
protected void del_door(string str, object who, object env, string *ye, string *yd)
{
	...
	if(!BANGHUI_D->parse_set_value(ref dcontent, r_dir[str], ref df_sect, ref de_sect, "create_door"))
	{
	...
	if(!BANGHUI_D->parse_set_value(ref content, str, ref f_sect, ref e_sect, "create_door"))
	{
