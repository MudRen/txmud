...
int vote(object me, object victim)
{
	...
	if( df > 0 )
	{
		shout( sprintf(HIM "【表决】%s 投票关闭 %s 的交谈频道，共需 %d 票，还差 %d 票。\n" NOR,
			me->name(),victim->name(),vv,df));
		write( sprintf(HIM "【表决】你投票关闭 %s 的交谈频道，还差 %d 票。\n" NOR,
			victim->name(),df));
	}

	else
	{
		shout(sprintf(HIM "【表决】%s 投票关闭 %s 的交谈频道。%s 的交谈频道被关闭了！\n" NOR,
			me->name(),victim->name(),victim->name()));
		write( sprintf(HIM "【表决】你投票关闭 %s 的交谈频道。%s 的交谈频道被关闭了！\n" NOR,
