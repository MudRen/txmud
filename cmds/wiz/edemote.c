...
int main(object me, string arg)
{
	...
	if( sscanf(arg, "-d %s", arg) ) {
		if(!EMOTE_D->delete_emote(arg))
			return notify_fail(sprintf("不存在 '%s' 这个 emote。\n",arg));
		write(sprintf("成功删除 emote：%s。\n",arg));
		log_file("emote",sprintf("%s(%s)将 %s 从 emote 列表中删除。\n",
			this_player()->name(),this_player()->query("id"),arg));
		return 1;
	}
...
int get_msg_others_target(string msg, mapping emote, string pattern)
{
	if (msg==".") {
		...
		log_file("emote",sprintf("%s(%s)将 %s 加入 emote 列表。\n",
			this_player()->name(),this_player()->query("id"),pattern));
		write("Emote 编辑结束。\n");
		return 1;
	}
